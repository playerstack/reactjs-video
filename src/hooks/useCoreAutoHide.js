...
export function useCoreAutoHide({ controllerRef, shouldStayVisible, delay = 3000, tapToToggle = false }) {
  ...
  React.useEffect(() => {
    ...
    // Any pointer activity reveals + restarts the countdown (unless locked).
    const onActivity = () => {
      if (stayVisibleRef.current) {
        ui.lock();
      } else {
    ...
    // Leaving the player hides immediately (unless forced visible or in tap-to-toggle mode —
    // on mobile, pointerleave fires when the finger lifts, which would undo the tap-to-show).
    const onLeave = () => {
      if (!stayVisibleRef.current && !tapToToggle) ui.hide();
    };
